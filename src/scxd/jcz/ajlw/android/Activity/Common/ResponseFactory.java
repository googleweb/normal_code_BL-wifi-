...
public class ResponseFactory {
	...
	private static String namessss;
				...
				while (itersElIterator.hasNext()) {
					...
					try {
						...
						FileUtil.writeToFile(
								URLedecoder_Str(itemEle.elementTextTrim(s
										.trim())),
	...
	private static Object decode_18C49(String xml) {
		...
		/*xml = AssestToString.xmltoString(MyApplication.getAppContext(),
				"xx.xml");*/
		resultMap = new HashMap<String, String>();
		if ("".equals(xml)) {
			resultMap.put("code", "0");
		...
		try {
			...
			if (iterbody.hasNext()) {
				...
				for (Element item : items) {
					...
					for (Element element : vals) {
						String value = "";
						namessss = element.getName();
						if (element.getName().equals("hphm")) {
							resultMap.put(element.getName(),
									isHZ(URLDecoder_Str(element
											.getStringValue().trim())));
						} else if (element.getName().equals("csys")) {
							String csyss = element.getStringValue();
							if ("null".equals(csyss) || "".equals(csyss)) {
							} else {
								char[] chars = csyss.toCharArray();
								if (chars.length > 0) {
